...
import type { Attendance, AttendanceInsert, AttendanceUpdate } from '../utils/supabase'
...
export const getUserAttendance = async (gameDayId: string, userId: string): Promise<Attendance | null> => {
	try {
...
export const getGameDayAttendances = async (gameDayId: string): Promise<Attendance[]> => {
	try {
		const { data, error } = await supabase.from('attendances').select('*').eq('game_day_id', gameDayId)
...
export const createAttendance = async (attendance: AttendanceInsert): Promise<Attendance | null> => {
	try {
		const { data, error } = await supabase.from('attendances').insert(attendance).select().single()
...
export const updateAttendance = async (id: string, updates: AttendanceUpdate): Promise<Attendance | null> => {
	try {
