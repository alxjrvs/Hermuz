...
export default async (interaction: ChatInputCommandInteraction) => {
	try {
		...
		if (serverGames.length === 0) {
			return interaction.editReply('No games have been set up in this server yet. Use `/game setup` to add a game.')
		}
		...
		const embed = new EmbedBuilder()
			...
			.setColor('#0099ff')
			.setDescription('Here are all the games that have been set up in this server:')
			.setTimestamp()
		...
		if (interaction.deferred || interaction.replied) {
			await interaction.editReply('An error occurred while listing games. Please try again later.')
		} else {
			await interaction.reply({
				content: 'An error occurred while listing games. Please try again later.',
